...
use ratatui::{
    prelude::*,
    widgets::{List, Row, StatefulWidget, Block, Paragraph},
    layout::{Constraint, Margin},
};
use crossterm::event::{self, Event, KeyCode, KeyEvent, KeyEventKind, MouseEventKind};

use crate::morse::{decode_symbols, encode_character};
...
struct WidgetState<'a> {
    ring: crate::ring::RingBufferIter<'a>,
}
...
impl Widget for &Decoder {
        ...
        where Self: Sized 
    {
        ...
        let list_area = Rect::new(0, 0, 100, 10).offset(offset.clone());
        Widget::render(list, list_area, buf);

        let mut lock: std::sync::RwLockReadGuard<State> = self.state.read().unwrap();
